...
def home(request): 
     ...
     if searchP:
          pendings = Pendings.objects.filter(
               Q(date__icontains = searchP) |
               Q(required_by__username__icontains = search) |
               Q(service_for__name__lastname__icontains = search) |
               Q(device__brand__model__icontains = search) |
               Q(last_revision__icontains = search) |
               Q(reason__icontains = search) |
               Q(tech_revision__name__icontains = search) |
               Q(state__status__icontains = search) 
          ).distinct()
