...
def dev_manager(request):
     ...
     if search:
          device = Device.objects.filter(
               ...
               Q(tech_revision__name__icontains = search) |
...
def dev_manager_add(request):
     return render(request, 'appsmartip/device_manager/device_add.html')
...
def dev_manager_add(request):
     ...
     if request.method == 'POST':
          add_device_form = DeviceFormAdd(data=request.POST, files=request.FILES)
...
def dev_manager_edit(request, id):
     ...
     if request.method == 'POST':
          edit_device_form = DevModelFormAdd(data=request.POST, instance=device)
