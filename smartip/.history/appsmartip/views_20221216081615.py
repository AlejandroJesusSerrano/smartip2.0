...
def add_dev_type(request):
     ...
     if request.method == 'POST':
          add_dev_type = DevTypeFormAdd(data=request.POST, files=request.FILES)
          if add_dev_type.is_valid():
               add_dev_type.save()
               messages.success(request, 'Tipo de dipositivo Agregado correctamente')
               return redirect(to='AdminDbDevModel')
          else:
               data['add_dev_type_form'] = add_dev_type

     return render(request, 'appsmartip/admin_db/device_type/admin_db_dev_type.html', data)
     
     # try:
     #      dev_type_add = request.POST['dev_type_form_input']
     #      dev_type_db_data = DevType(dev_type_form_input=dev_type_add)

     #      if dev_type_add == '':
     #           raise ValueError('El campo no puede estar vacío.')

     #      dev_type_db_data.save()
     #      return HttpResponseRedirect(reverse('AdminDbDevType'))
...
def delete_dev_type(request, id):
     dev_type_db_data = DevType.objects.filter(id=id)
     dev_type_db_data.delete()
     messages.success(request, 'Eliminado correctamente')
     return HttpResponseRedirect(reverse("AdminDbDevType"))

# Edit Dev Type
def update_dev_type(request, id):
     dev_type_db_data = DevType.objects.all()
     dev_type_db_data_only = DevType.objects.get(pk=id)
     data = {
          'dev_type_db_data': dev_type_db_data,
     }
     return render(request, 'appsmartip/admin_db/device_type/admin_db_dev_type.html', data)
