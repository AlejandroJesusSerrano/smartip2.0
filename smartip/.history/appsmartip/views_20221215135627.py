...
def admin_db_dev_type(request, id):
     dev_type = DevType.objects.all()
     dev_type_edit = get_object_or_404(DevType, id=id)

     data = {
          'dev_type': dev_type,
          'dev_type_edit': dev_type_edit,
          'add_dev_type': DevTypeFormAdd(),
          'edit_dev_type': DevTypeFormAdd(instance=dev_type_edit)
     }

     # Add Dev Type
     if request.method == 'POST':
          add_dev_type_form = DevTypeFormAdd(data=request.POST)
          if add_dev_type_form.is_valid():
               add_dev_type_form.save()
               messages.success(request, 'Modelo Agregado correctamente')
          else:
               data['add_dev_type'] = add_dev_type_form

     # Edit Dev Type
     if request.method == 'POST':
          edit_dev_type_form = DevModelFormAdd(data=request.POST, instance=dev_type_edit)
          if edit_dev_type_form.is_valid():
               edit_dev_type_form.save()
               messages.success(request, 'Modificado correctamente')
          else:
               data['edit_dev_type'] = edit_dev_type_form
...
def admin_db_delete_dev_type(request, id):
