...
def add_dev_type(request):
     try:
          dev_type_add = request.POST['dev_type_input']
          dev_type_db_data = DevType(dev_type_input=dev_type_add)
          is_in = DevType.objects.filter(dev_type_add__iexact=dev_type_db_data).exists()

          if dev_type_add == '':
               raise ValueError('El campo no puede estar vacío.')
          if is_in:
               raise ValidationError('El tipo de dispositivo, ya se encuntra en la base de datos')
