...
def admin_db_pen_status(request):
     ...
     return render(request, 'appsmartip/admin_db/device_status/admin_db_pen_status.html', data)
...
def add_pen_status(request):
     ...
     if request.method == 'POST':
          pending_status_add = DevStatusFormAdd(data=request.POST)
     ...
     return render(request, 'appsmartip/admin_db/device_status/admin_db_pen_status_add.html', data)

#* Pending Status Edit
permission_required('appsmartip.modify_pending_status')
def edit_pending_status(request, id):
     dev_status = get_object_or_404(PendingStatus, id=id)
     data = {
          'pending_status_edit_form': PendingStatusFormAdd(instance=dev_status)
     }

     if request.method == 'POST':
          pending_status_edit = PendingStatusFormAdd(request.POST, instance=dev_status)
     ...
     return render(request, 'appsmartip/admin_db/device_status/admin_db_pen_status_edit.html', data)

#* Pending Status Delete
permission_required('appsmartip.delete_pending_status')
def delete_pending_status(request, id):
     pending_status = get_object_or_404(DevStatus, id=id)
