...
class OfficeFormAdd(forms.ModelForm):
     ...
     def clean_court__office_name(self):
          court = self.cleaned_data['court']
          office_name = self.cleaned_data['office_name']
          is_in_court = Office.objects.filter(court_iexact=self.court).exists()
          is_in_office_name = Office.objects.filter(office_name_iexact=self.office_name).exists()
          
          if is_in_court and is_in_office_name:
               raise ValidationError('La oficina ya se encuentra incorporada a la base de datos')
          
          return court and office_name
