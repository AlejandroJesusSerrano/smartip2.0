...
class OfficeFormAdd(forms.ModelForm):
     ...
     def clean_Office(self): 
          if Office.objects.filter(location_iexact=self.location, court__iexact=self.court, office_name_iexact=self.office_name).exists():
               raise ValidationError('Esta oficina ya se encuentra en la Base de Datos')
     ...
     class Meta:
          ...
          constraints = {
               models.UniqueConstraint(fields=['location', 'edifice', 'floor', 'court', 'office_name'], name='La Oficina ya se encuentra agregada a la Base de datos')
          }
