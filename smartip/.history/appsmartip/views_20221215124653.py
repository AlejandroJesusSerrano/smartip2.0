...
# List Types
@permission_required('appsmartip.view_dev_type' and ('appsmartip.add_dev_type' or 'appsmartip.modify_dev_type')) 
def admin_db_dev_type(request):
     dev_type = DevType.objects.all()

     data = {
          'dev_type': dev_type,
          'admin_db_dev_type_form': DevTypeFormAdd()
     }

     if request.method == 'POST':
          admin_db_dev_type_form = DevTypeFormAdd(data=request.POST)
          if admin_db_dev_type_form.is_valid():
               admin_db_dev_type_form.save()
               messages.success(request, 'Modelo Agregado correctamente')
               return redirect(to='AdminDbDevModel')
          else:
               data['admin_db_dev_type_form'] = admin_db_dev_type_form
...
# def admin_db_dev_type_add(request):
#      data = {
#           'add_dev_type_form': DevTypeFormAdd()
#      }

#      if request.method == 'POST':
#           add_dev_type_form = DevModelFormAdd(data=request.POST, files=request.FILES)
#           if add_dev_type_form.is_valid():
#                add_dev_type_form.save()
#                messages.success(request, 'Modelo Agregado correctamente')
#                return redirect(to='AdminDbDevModel')
#           else:
#                data['add_dev_type_form'] = add_dev_type_form

#      return render(request, 'appsmartip/admin_db/device_type/admin_db_dev_type.html', data)

# Edit Device Type
def admin_db_dev_type_edit(request, id):
     dev_type_add = DevType.objects.all()
     data = {
          'dev_type_add': dev_type_add
     }

     return render(request, 'appsmartip/device_type/admin_db_dev_type.html', data)
