...
def list_ips(request):
     ...
     if search:
          ips = Ip.objects.filter(
               Q(ipdir__icontains = search) |
               Q(internet_access__access_type__icontains = search) 
          ).distinct()     
...
def add_ip(request):
     ...
     if request.method == 'POST':
          ...
          if ip_add.is_valid():
               ip_add.save()
               messages.success('Direccion IP agregada correctamente')
...
def edit_access(request, id):
     ...
     if request.method == 'POST':
          ...
          if edit_access_type.is_valid():
               ...
               messages.success('Tipo de accesso editado exitosamente')
               return redirect(to='InAccessList')
          else:
